...
class Generator(nn.Module):
    ...
    def __init__(self, device, z_dim=256, z_dim_bg=128, decoders=[],
        ...
        # (NEW) mask volume rendering of foreground objects outside bounds
        self.mask_foreground_outside = mask_foreground_outside
        # (NEW) gradually fade in the background, by iteration
    ...
    def forward(self, batch_size=32, latent_codes=None, camera_matrices=None,
                transformations=None, bg_rotation=None, mode="training", it=None,
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
                            ...
                            it=None, return_alpha_map=False,
                            not_render_background=False,
                            only_render_background=False):
        ...
        for i in range(n_iter):
            if i < n_boxes:  # Object
                ...
                if mode == 'training' and it is not None:
                    # gradually fade background into scene
                    fade_factor = it / self.fade_length if it <= self.fade_length else 1.
                    feat_i = fade_factor*feat_i
                    sigma_i = fade_factor*sigma_i
